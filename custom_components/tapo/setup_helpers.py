...
class TapoAddressFinder:
    ...
    async def lookup(self) -> str:
        if (
            ...
            if self._config.data.get(CONF_MAC, None) is not None:
                return await self._try_track_ip_address(
                    self._config.data.get(CONF_MAC), self._config.data.get(CONF_HOST)
                )
            else:
                logging.warning(
    ...
    async def _try_track_ip_address(self, mac: str, last_known_ip: str) -> str:
        _LOGGGER.info(

...
from custom_components.tapo.hub.tapo_hub_child_coordinator import TapoHubChildCoordinator
...
class TestTapoHubChildCoordinator:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("device_type, expected_device_state, expected_nickname",
    ...
    async def test_setup_child_coordinators_should_create_correct_types(self, device_type: HubChildDevice,
        ...
        with patch("homeassistant.helpers.update_coordinator.DataUpdateCoordinator.async_config_entry_first_refresh"):
            ...
            hub_child_coordinator = TapoHubChildCoordinator(hass=self.hass, device=device, polling_interval=self.polling_rate)
    ...
    async def test_setup_child_coordinators_should_create_correct_types_get_device_info(self, device_type: HubChildDevice,
                                                                         expected_device_state: HubChildCommonState,
                                                                         expected_nickname: str):
        with patch("homeassistant.helpers.update_coordinator.DataUpdateCoordinator.async_config_entry_first_refresh"):
            device = AsyncMock(device_type)
